...
- **What:** every asteroid rotates while it flies.  Two new frames
  (`rock1.spr` +20°, `rock2.spr` −20°, generated by the new
  `games/dodge/tools/rock_spin.py`) are emitted into the two free
  sprite slots at $3880/$38C0 (pointers $E2/$E3) — immediately before
  the sound presets at $3900, with a build guard that errors on
  overlap.  At the tail of `ast_update` a 10-frame cadence advances
  `ast_spin` (mod 3), and a small per-slot loop writes
  `rock_ptrs[(ast_spin + slot) mod 3]` to $07FA–$07FC, so multiple
  rocks tumble out of phase.  `restart()` resets the two new vars.
  Docs synced: behaviors.yaml (System 10 "Rock rotation" + variable
  table), sprites.yaml memory map, ASSEMBLY.md.
- **Why:** a motionless rock read as a static hazard; the tumbling
  silhouette (approved via GIF preview at the 200 ms cadence) reads
  as tumbling mass and telegraphs "moving object" at a glance.
- **Verification:** live monitor tape — phase cycles 0→1→2 with
  10-frame holds; all three pointers match `rock_ptrs[(spin+slot)%3]`
  across frames; frames byte-verified at $3880/$38C0 in the PRG.
  The live test suite's remaining failures are pre-existing harness
  flakiness (identical failure set reproduced on the pre-change
