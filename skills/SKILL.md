...
### Sound Presets (`macros/sound_presets.acme`)

Ready-to-use sound effect subroutines.  Included automatically in every project.  Requires four variables (auto-generated by `c64devk build`):

```
sfx_dur:   !byte 0    ; frame countdown → gate-off when zero
sfx_step:  !byte 0    ; step counter for multi-frame sounds
sfx_tmp:   !byte 0    ; scratch / sub-timer
sfx_kind:  !byte 0    ; 0=none, 1=coin, 2=alarm, 3=laser, 4=powerup
```

**Single-shot presets** — call JSR, sound plays, gate_off cleans up:

| Subroutine | Voice | Waveform | Duration | Description |
|-----------|-------|----------|----------|-------------|
| `sfx_beep` | 1 | triangle | 6 frames | High-pitched blip (~962 Hz) |
| `sfx_boop` | 1 | triangle | 10 frames | Low-pitched blip (~241 Hz) |
| `sfx_hit` | 3 | noise | 4 frames | Short noise impact |
| `sfx_explosion` | 3 | noise | 25 frames | Noise rumble |

**Multi-frame presets** — JSR to start, then `sfx_tick_all` each frame dispatches to the active one:

| Subroutine | Voice | Waveform | Behaviour |
|-----------|-------|----------|-----------|
| `sfx_coin` | 1 | triangle | Two-note ding: E5 → A5, 6 frames each |
| `sfx_alarm` | 2 | triangle | Alternating hi/lo every 15 frames |
| `sfx_laser` | 2 | saw | Descending frequency sweep, ~30 frames |
| `sfx_powerup` | 1 | triangle | Ascending 4-note arpeggio (~241→1926 Hz) |

**Housekeeping subroutines** — call each frame:

| Subroutine | Purpose |
|-----------|---------|
| `sfx_gate_off` | Decrements `sfx_dur`, gates off voice 1+3 when zero |
| `sfx_tick_all` | Dispatches to active multi-frame tick via `sfx_kind` |
| `sfx_silence` | Gates off all 3 voices immediately |

**Usage pattern in `game_logic.acme`:**
```asm
    ;; Single-shot:
    jsr sfx_beep

    ;; Multi-frame (in your game loop):
    lda frame_ready     ; (or equivalent frame-tick)
    beq .skip
    jsr sfx_gate_off
    jsr sfx_tick_all
```

For raw SID register access, use the `SID_*` constants directly.  For note-name-to-frequency
conversion, use the framework's `play_sound` behavior action.
...
- `sfx_dur` — byte, frame countdown for single-shot sound presets
- `sfx_step` — byte, step counter for multi-frame sound presets
- `sfx_tmp` — byte, scratch timer for multi-frame sound presets
- `sfx_kind` — byte, active multi-frame preset ID (0=none, 1=coin, 2=alarm, 3=laser, 4=powerup)
